...
class TheMovieDBService {
    ...
    // MARK: - Get Details
    func fetchPopular(movie_id: Int, onComplete: @escaping (MovieDetails?) -> Void) {
        let url: String
        url = "\(API_BASE)\(MOVIE)\(movie_id)?api_key=\(API_KEY)"
        print("DEBUG: url Details...: \(url)")
        ...
        Alamofire.request(url).responseJSON { (response) in
            ...
            do {
                let movieDetailInfo = try JSONDecoder().decode(MovieDetails.self, from: data)
                onComplete(movieDetailInfo)
            } catch {
                print(error.localizedDescription)
                onComplete(nil)
            }
    ...
    func fetchPopular(movie_id: Int, page: Int, onComplete: @escaping (Similar?) -> Void) {
        ...
        Alamofire.request(url).responseJSON { (response) in
            ...
            do {
                let similarInfo = try JSONDecoder().decode(Similar.self, from: data)
                onComplete(similarInfo)
            } catch {
                print(error.localizedDescription)
                onComplete(nil)
            }
